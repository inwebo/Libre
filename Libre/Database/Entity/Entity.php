...
namespace Libre\Database {
    ...
    abstract class Entity
    {
         ...
         */
        static public function setConfiguration(DriverInterface $iDriver, $primaryKey = null, $tableName = null)
        {
            $driver = $iDriver;
        ...
        static protected function prepareDefaultCrudQueries()
        {
            ...
            $select = sprintf(
                self::SQL_SELECT,
                self::getConfiguration()->getTable(),
                self::getConfiguration()->getPrimaryKey()
            );
            $delete = sprintf(
                self::SQL_DELETE,
                self::getConfiguration()->getTable(),
            ...
            $insert = sprintf(
                ...
                self::getConfiguration()->getTable(),
                $aggregatedColsName,
                $aggregatedColsTokens
            );
            $update = sprintf(
                self::SQL_UPDATE,
                self::getConfiguration()->getTable(),
                $aggregatedColsUpdate,
                self::getConfiguration()->getPrimaryKey()
            );
        ...
        public function save()
        {
            ...
            if ($this->isLoaded()) {
                ...
                $values = array_merge($colsValues, [$id]);
                ...
                $cols = array_merge([$this->getConfiguration()->getPrimaryKey() => null], $cols);
